...
async function getDb(): Promise<Db> {
  ...
  await database.collection(ROASTS_COLLECTION).createIndexes([
    { key: { userId: 1 }, name: "idx_roasts_userId" },
    { key: { createdAt: -1 }, name: "idx_roasts_createdAt" },
  ]);

  return database;
}

export async function saveRoastRecord(input: RoastRecordInput): Promise<void> {
  const database = await getDb();
  await database.collection(ROASTS_COLLECTION).insertOne({
    userId: input.userId,
...
export async function getUniqueUserCount(): Promise<number> {
  const database = await getDb();
  const result = await database.collection(ROASTS_COLLECTION).aggregate([
    { $group: { _id: "$userId" } },
    { $count: "count" },
  ]).toArray();
...
export async function getRoastsPerUser(): Promise<Array<{ userId: string; count: number }>> {
  ...
  return database.collection(ROASTS_COLLECTION).aggregate([
    { $group: { _id: "$userId", count: { $sum: 1 } } },
    { $sort: { count: -1, _id: 1 } },
    { $project: { _id: 0, userId: "$_id", count: 1 } },
  ]).toArray() as Promise<Array<{ userId: string; count: number }>>;
}
...
export async function getDailyUniqueUsers(): Promise<Array<{ date: string; count: number }>> {
  const startOfWindow = new Date();
  startOfWindow.setDate(startOfWindow.getDate() - 6);
  startOfWindow.setHours(0, 0, 0, 0);
  ...
  return database.collection(ROASTS_COLLECTION).aggregate([
    ...
    { $sort: { date: 1 } },
  ]).toArray() as Promise<Array<{ date: string; count: number }>>;
}

export async function getRoastAnalytics(): Promise<{
  totalRoasts: number;
  uniqueUsers: number;
  roastsPerUser: Array<{ userId: string; count: number }>;
  roastsGeneratedToday: number;
  roastsGeneratedLast7Days: number;
  dailyUniqueUsers: Array<{ date: string; count: number }>;
}> {
  const [totalRoasts, uniqueUsers, roastsPerUser, roastsGeneratedToday, roastsGeneratedLast7Days, dailyUniqueUsers] = await Promise.all([
    getTotalRoasts(),
    getUniqueUserCount(),
    getRoastsPerUser(),

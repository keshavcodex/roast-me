...
import type { AudioSessionResponse, LiveRoastCallbacks, LiveRoastPhase } from "@/types/live";
...
export class LiveRoastController {
  ...
  async start(message: string, intensity: number): Promise<void> {
    ...
    if (!this.isSessionResponse(payload)) throw new LiveRoastError("Audio roast setup returned an invalid response.");
    ...
    this.session = await ai.live.connect({
      ...
      callbacks: {
        ...
        onclose: () => {
          if (!this.generationComplete && !this.hasAudio) this.callbacks.onError("Audio connection ended early.");
        },
      },
    });

    this.setPhase("roasting");
    this.session.sendRealtimeInput({ text: `Roast intensity: ${intensity}/5. User's excuse: "${message}". Respond now.` });
  }
  ...
  play(): void {
    void this.audioContext.resume().then(() => {
      if (this.audioContext.state === "running") this.setPhase(this.generationComplete ? "playing" : "roasting");
  ...
  private handleMessage(event: LiveServerMessage): void {
    ...
    content.modelTurn?.parts?.forEach((part) => {
      if (part.inlineData?.data) this.queueChunk(decodePcm(part.inlineData.data), true);
    });
  ...
  private queueChunk(chunk: Float32Array, shouldStore: boolean): void {
    ...
    if (!this.hasAudio) {
      ...
      if (this.audioContext.state !== "running") this.callbacks.onAutoplayBlocked();
    }
  ...
  private finishWhenPlaybackEnds(): void {
    ...
    const waitMs = Math.max(0, (this.nextStartTime - this.audioContext.currentTime) * 1000);
    window.setTimeout(() => {
  ...
  private trackProgress = (): void => {
    ...
    const elapsed = Math.max(0, this.audioContext.currentTime - this.playStartTime);

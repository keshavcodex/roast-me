...
export class LiveRoastController {
	...
	constructor(
		private readonly callbacks: LiveRoastCallbacks
	) {
	...
	async start(
		...
		if (!this.isSessionResponse(payload)) {
			throw new LiveRoastError(
				'Audio response setup returned an invalid response.'
			);
		}
...
Respond now according to the selected mode.
`
		});
	}
	...
	private handleMessage(
		...
		content.modelTurn?.parts?.forEach(
			(part) => {
		...
		if (
			...
			void this.callbacks.onGenerationComplete?.();
	...
	private queueChunk(
		...
		const audioData =
			new Float32Array(
				new ArrayBuffer(chunk.length * 4)
			);
	...
	private finishWhenPlaybackEnds(): void {
		if (!this.hasAudio) {
			this.callbacks.onError(
				'Gemini returned no playable audio.'
			);
			return;
		}
	...
	private isSessionResponse(
		value: unknown
	): value is AudioSessionResponse {
		return (
			typeof value === 'object' &&
			value !== null &&
			typeof (value as AudioSessionResponse)
				.token === 'string' &&
			typeof (value as AudioSessionResponse)
				.model === 'string'
		);
